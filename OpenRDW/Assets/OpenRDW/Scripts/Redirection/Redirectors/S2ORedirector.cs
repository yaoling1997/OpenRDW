...
public class S2ORedirector : SteerToRedirector {
    ...
    {        
        ...
        var S2O_TARGET_RADIUS = 5f;//Target orbit radius for Steer-to-Orbit algorithm (meters)
        var trackingSpaceSize = redirectionManager.globalConfiguration.GetTrackingSpaceBoundingboxSize();
        if (trackingSpaceSize.x <= 10 || trackingSpaceSize.y <= 10)
        {
            //S2O_TARGET_RADIUS = 2f;
            S2O_TARGET_RADIUS = Mathf.Min(trackingSpaceSize.x, trackingSpaceSize.y) / 5;
